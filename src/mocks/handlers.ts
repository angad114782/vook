...
const resolver: HttpResponseResolver = async ({ request }) => {
  ...
  if (path === '/payroll-runs' && method === 'POST') {
    ...
    if ((state.payrollRuns as Row[]).some((item) => item.companyId === currentUser?.companyId && item.month === Number(body.month) && item.year === Number(body.year) && item.status !== 'CANCELLED')) return fail(409, 'PAYROLL_PERIOD_EXISTS', 'A payroll run already exists for this period.');
    const created = await updateMockState((draft) => {
      const payslips = (draft.payslips as Row[]).filter((item) => item.companyId === currentUser?.companyId && item.month === Number(body.month) && item.year === Number(body.year));
      const run = { id: id('payroll_run'), companyId: currentUser?.companyId, month: Number(body.month), year: Number(body.year), status: 'CALCULATED', preparedBy: currentUser?.id, approvedBy: null, finalizedAt: null, publishedAt: null, payslipIds: payslips.map((item) => item.id), totalNetMinor: payslips.reduce((sum, item) => sum + Math.round(Number(item.netPay ?? 0) * 100), 0), currency: 'INR', createdAt: new Date().toISOString() };
      draft.payrollRuns.unshift(run);
      appendAudit(draft, { actorId: currentUser?.id, companyId: currentUser?.companyId, action: 'PAYROLL_CALCULATED', entityType: 'PAYROLL_RUN', entityId: run.id, newValue: run });
      return { message: 'Payroll calculated and ready for review.', period: `${body.month}/${body.year}`, month: body.month, year: body.year, created: state.employees.filter((item) => item.companyId === currentUser?.companyId).length, skipped: 0, runId: run.id, status: run.status };
    });
  ...
  if (payrollAction && method === 'POST') {
    ...
    const transitions: Record<string, { from: string[]; to: string; permission: string }> = {
      review: { from: ['CALCULATED'], to: 'UNDER_REVIEW', permission: 'PAYROLL.PROCESS' },
    ...
    const updated = await updateMockState((draft) => {
      ...
      appendAudit(draft, { actorId: currentUser?.id, companyId: payroll.companyId, action: `PAYROLL_${transition.to}`, entityType: 'PAYROLL_RUN', entityId: payroll.id, oldValue: before, newValue: payroll });
      return payroll;
    });
    return ok(updated);
  }
