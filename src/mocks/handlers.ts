...
const resolver: HttpResponseResolver = async ({ request }) => {
  ...
  if (currentUser) {
    ...
    if ((path.startsWith('/role-definitions') || path.startsWith('/role-assignments') || path.startsWith('/role-permissions')) && role !== 'COMPANY_ADMIN' && !(role === 'SUPER_ADMIN' && method === 'GET')) return fail(403, 'FORBIDDEN', 'Only the Company Admin can configure tenant roles and scopes.');
    ...
    if (role !== 'SUPER_ADMIN' && currentUser.companyId) {
      const moduleRules: Array<[RegExp, string]> = [
        ...
        [/^\/attendance-verification-policy/, 'ATTENDANCE'], [/^\/attendance-integrations/, 'ATTENDANCE'], [/^\/attendance-regularizations/, 'ATTENDANCE'], [/^\/attendance-periods/, 'ATTENDANCE'], [/^\/attendance/, 'ATTENDANCE'], [/^\/shifts/, 'SHIFT_MANAGEMENT'], [/^\/leave-requests|^\/holiday-calendars/, 'LEAVE_MANAGEMENT'],
        [/^\/approvals|^\/workflows/, 'APPROVALS'], [/^\/salaries|^\/payroll-runs|^\/payroll-compliance/, 'PAYROLL'], [/^\/payslips/, 'PAYSLIPS'],
      ...
      if (rule) {
        ...
        const domainAction = path.includes('/actions') || /^\/payroll-runs\/[^/]+\/(review|approve|finalize|publish)$/.test(path) || (/^\/(approvals|expenses)\/[^/]+$/.test(path) && method === 'PATCH') ? null : path.includes('/finalize') ? 'FINALIZE' : path === '/payroll-runs' && method === 'POST' ? 'PROCESS' : action;
  ...
  if (employeeMatch) { const found = state.employees.find((item) => item.id === employeeMatch[1]) as Row | undefined; if (!found) return fail(404, 'NOT_FOUND', 'Employee not found.'); if (currentUser?.role !== 'SUPER_ADMIN' && !recordWithinScope(state, currentUser, found, method === 'GET' ? 'EMPLOYEE_MANAGEMENT.VIEW' : 'EMPLOYEE_MANAGEMENT.EDIT')) return fail(403, 'SCOPE_DENIED', 'This employee is outside your assigned organizational scope.'); if (method === 'GET') return ok(employeeView(state, found)); const body = await bodyOf(request); if (employeeMatch[2] === 'actions') { const action = String(body.action || '').toUpperCase(); const allowed: Record<string, string[]> = { ACTIVATE: ['DRAFT', 'INVITED', 'PREBOARDING', 'ONBOARDING', 'PROBATION', 'SUSPENDED'], START_NOTICE: ['ACTIVE', 'PROBATION'], SUSPEND: ['ACTIVE', 'PROBATION'], EXIT: ['NOTICE_PERIOD', 'RESIGNATION'], ARCHIVE: ['EXITED'] }; if (!allowed[action]?.includes(String(found.status).toUpperCase())) return fail(409, 'INVALID_EMPLOYEE_TRANSITION', `Employee cannot move from ${found.status} using ${action}.`); if (Number(body.version) !== Number(found.version ?? 1)) return fail(409, 'VERSION_CONFLICT', 'This employee record changed after you opened it. Refresh and try again.'); if (!String(body.reason ?? '').trim()) return fail(422, 'AUDIT_REASON_REQUIRED', 'Add a reason for this lifecycle change.'); } const updated = await updateMockState((draft) => { const row = draft.employees.find((item) => item.id === employeeMatch[1]) as Row; const before = structuredClone(row); if (employeeMatch[2] === 'actions') { const action = String(body.action || '').toUpperCase(); const transitions: Record<string, string> = { ACTIVATE: 'ACTIVE', SUSPEND: 'SUSPENDED', START_NOTICE: 'NOTICE_PERIOD', EXIT: 'EXITED', ARCHIVE: 'ARCHIVED' }; row.status = transitions[action] ?? row.status; if (action === 'EXIT' && row.userId) { const user = draft.users.find((item) => item.id === row.userId); if (user) user.isActive = false; } } else if (employeeMatch[2] === 'account') (row.user as Row).accountStatus = 'INVITED'; else Object.assign(row, body); row.version = Number(row.version || 0) + 1; appendAudit(draft, { actorId: currentUser?.id, companyId: row.companyId, action: employeeMatch[2] === 'actions' ? `EMPLOYEE_${String(body.action).toUpperCase()}` : 'EMPLOYEE_UPDATED', entityType: 'EMPLOYEE', entityId: row.id, oldValue: before, newValue: row, reason: body.reason ?? null }); return employeeView(draft, row); }); return ok(updated); }
  ...
  if (path === '/attendance' && method === 'GET') { const month = Number(url.searchParams.get('month') || 0); const year = Number(url.searchParams.get('year') || 0); const search = String(url.searchParams.get('search') || '').toLowerCase(); const rows = (state.attendance as Row[]).filter((item) => { const date = new Date(item.date); const employee = (state.employees as Row[]).find((candidate) => candidate.id === item.employeeId); const name = String((employee?.user as Row)?.name ?? '').toLowerCase(); return (!month || date.getMonth() + 1 === month) && (!year || date.getFullYear() === year) && (!search || name.includes(search) || String(employee?.employeeId ?? '').toLowerCase().includes(search)); }).map((item) => attendanceView(state, item)); const result = page(rows, url); return ok({ records: result.rows, pagination: result.pagination }); }
  if (path === '/attendance' && method === 'POST') {
    const body = await bodyOf(request);
    const date = new Date(`${String(body.date).slice(0, 10)}T00:00:00`);
    const locked = (state.attendancePeriods as Row[]).find((item) => item.companyId === currentUser?.companyId && item.month === date.getMonth() + 1 && item.year === date.getFullYear() && item.status === 'LOCKED');
    if (locked) return fail(409, 'ATTENDANCE_PERIOD_LOCKED', 'This attendance period is locked. Use the regularization workflow for corrections.');
    const created = await updateMockState((draft) => { const row = { id: id('attendance'), companyId: currentUser?.companyId, source: 'MANUAL', ...body }; draft.attendance.unshift(row); return attendanceView(draft, row); });
    return ok(created, 201);
  }
  if (path === '/attendance-periods/current' && method === 'GET') { const company = companyFor(state, currentUser, url); const month = Number(url.searchParams.get('month')); const year = Number(url.searchParams.get('year')); const found = (state.attendancePeriods as Row[]).find((item) => item.companyId === company?.id && item.month === month && item.year === year); return ok(found ?? { id: `attendance_period_${company?.id}_${year}_${month}`, companyId: company?.id, month, year, status: 'OPEN', version: 0, lockedBy: null, lockedAt: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }); }
  if (path === '/attendance-periods/lock' && method === 'POST') { const body = await bodyOf(request); const company = companyFor(state, currentUser, url); const month = Number(body.month); const year = Number(body.year); if (!month || !year) return fail(422, 'VALIDATION_ERROR', 'Payroll month and year are required.'); const found = (state.attendancePeriods as Row[]).find((item) => item.companyId === company?.id && item.month === month && item.year === year); if (found?.status === 'LOCKED') return ok(found); if (found && Number(body.version) !== Number(found.version)) return fail(409, 'VERSION_CONFLICT', 'This attendance period changed in another session.'); const unresolved = (state.attendanceRegularizations as Row[]).filter((item) => item.companyId === company?.id && String(item.date).startsWith(`${year}-${String(month).padStart(2, '0')}`) && !['APPROVED', 'REJECTED'].includes(String(item.status).toUpperCase())); if (unresolved.length) return fail(409, 'ATTENDANCE_EXCEPTIONS_OPEN', `${unresolved.length} attendance regularization request${unresolved.length === 1 ? '' : 's'} must be resolved before locking.`, { count: unresolved.length }); const locked = await updateMockState((draft) => { const now = new Date().toISOString(); let period = (draft.attendancePeriods as Row[]).find((item) => item.companyId === company?.id && item.month === month && item.year === year); const before = period ? structuredClone(period) : null; if (!period) { period = { id: id('attendance_period'), companyId: company?.id, month, year, status: 'OPEN', version: 0, createdAt: now }; draft.attendancePeriods.unshift(period); } Object.assign(period, { status: 'LOCKED', version: Number(period.version) + 1, lockedBy: currentUser?.id, lockedAt: now, updatedAt: now }); appendAudit(draft, { actorId: currentUser?.id, companyId: company?.id, action: 'ATTENDANCE_PERIOD_LOCKED', entityType: 'ATTENDANCE_PERIOD', entityId: period.id, oldValue: before, newValue: period }); return period; }); return ok(locked); }
  ...
  if (path.startsWith('/holiday-calendars') && method !== 'GET') {
    const permissions = effectivePermissions(state, currentUser);
    if (!permissions.includes('*') && !permissions.includes('LEAVE_MANAGEMENT.CONFIGURE')) return fail(403, 'PERMISSION_DENIED', 'Holiday calendar changes require LEAVE_MANAGEMENT.CONFIGURE.');
  }

  if (path === '/holiday-calendars' && method === 'GET') {
    const company = companyFor(state, currentUser, url);
    const year = Number(url.searchParams.get('year') || 0);
    const calendars = (state.holidayCalendars as Row[]).filter((item) => item.companyId === company?.id && (!year || Number(item.year) === year));
    return ok({ calendars });
  }
  if (path === '/holiday-calendars' && method === 'POST') {
    const body = await bodyOf(request);
    if (!body.name || !Number(body.year)) return fail(422, 'VALIDATION_ERROR', 'Calendar name and year are required.');
    const company = companyFor(state, currentUser, url);
    const duplicate = (state.holidayCalendars as Row[]).some((item) => item.companyId === company?.id && Number(item.year) === Number(body.year) && item.name === body.name);
    if (duplicate) return fail(409, 'HOLIDAY_CALENDAR_EXISTS', 'A calendar with this name already exists for the selected year.');
    const created = await updateMockState((draft) => { const now = new Date().toISOString(); const calendar = { id: id('holiday_calendar'), companyId: company?.id, name: body.name, year: Number(body.year), stateCode: body.stateCode ?? null, branchIds: body.branchIds ?? [], employeeGroupIds: body.employeeGroupIds ?? [], holidays: [], version: 1, permittedActions: ['EDIT', 'COPY', 'PUBLISH'], createdAt: now, updatedAt: now }; draft.holidayCalendars.unshift(calendar); appendAudit(draft, { actorId: currentUser?.id, companyId: company?.id, action: 'HOLIDAY_CALENDAR_CREATED', entityType: 'HOLIDAY_CALENDAR', entityId: calendar.id, newValue: calendar }); return calendar; });
    return ok(created, 201);
  }
  const holidayCalendarMatch = path.match(/^\/holiday-calendars\/([^/]+)$/);
  if (holidayCalendarMatch && method === 'PATCH') {
    const body = await bodyOf(request); const found = (state.holidayCalendars as Row[]).find((item) => item.id === holidayCalendarMatch[1]);
    if (!found) return fail(404, 'NOT_FOUND', 'Holiday calendar not found.');
    if (Number(body.version) !== Number(found.version)) return fail(409, 'VERSION_CONFLICT', 'This calendar changed in another session. Refresh and try again.');
    const updated = await updateMockState((draft) => { const calendar = (draft.holidayCalendars as Row[]).find((item) => item.id === found.id); const before = structuredClone(calendar); Object.assign(calendar, body, { version: Number(calendar.version) + 1, updatedAt: new Date().toISOString() }); appendAudit(draft, { actorId: currentUser?.id, companyId: calendar.companyId, action: 'HOLIDAY_CALENDAR_UPDATED', entityType: 'HOLIDAY_CALENDAR', entityId: calendar.id, oldValue: before, newValue: calendar }); return calendar; });
    return ok(updated);
  }
  const holidayCreateMatch = path.match(/^\/holiday-calendars\/([^/]+)\/holidays$/);
  if (holidayCreateMatch && method === 'POST') {
    const body = await bodyOf(request); const found = (state.holidayCalendars as Row[]).find((item) => item.id === holidayCreateMatch[1]);
    if (!found) return fail(404, 'NOT_FOUND', 'Holiday calendar not found.');
    if (Number(body.version) !== Number(found.version)) return fail(409, 'VERSION_CONFLICT', 'This calendar changed in another session. Refresh and try again.');
    if (!body.name || !body.date) return fail(422, 'VALIDATION_ERROR', 'Holiday name and date are required.');
    if ((found.holidays ?? []).some((item: Row) => item.date === body.date)) return fail(409, 'HOLIDAY_DATE_EXISTS', 'A holiday already exists on this date.');
    const updated = await updateMockState((draft) => { const calendar = (draft.holidayCalendars as Row[]).find((item) => item.id === found.id); const before = structuredClone(calendar); calendar.holidays.push({ id: id('holiday'), name: body.name, date: body.date, kind: body.kind ?? 'COMPANY', optional: Boolean(body.optional) }); calendar.holidays.sort((a: Row, b: Row) => String(a.date).localeCompare(String(b.date))); calendar.version += 1; calendar.updatedAt = new Date().toISOString(); appendAudit(draft, { actorId: currentUser?.id, companyId: calendar.companyId, action: 'HOLIDAY_ADDED', entityType: 'HOLIDAY_CALENDAR', entityId: calendar.id, oldValue: before, newValue: calendar }); return calendar; });
    return ok(updated, 201);
  }
  const holidayDeleteMatch = path.match(/^\/holiday-calendars\/([^/]+)\/holidays\/([^/]+)$/);
  if (holidayDeleteMatch && method === 'DELETE') {
    const body = await bodyOf(request); const found = (state.holidayCalendars as Row[]).find((item) => item.id === holidayDeleteMatch[1]);
    if (!found) return fail(404, 'NOT_FOUND', 'Holiday calendar not found.');
    if (Number(body.version) !== Number(found.version)) return fail(409, 'VERSION_CONFLICT', 'This calendar changed in another session. Refresh and try again.');
    const updated = await updateMockState((draft) => { const calendar = (draft.holidayCalendars as Row[]).find((item) => item.id === found.id); const before = structuredClone(calendar); calendar.holidays = calendar.holidays.filter((item: Row) => item.id !== holidayDeleteMatch[2]); calendar.version += 1; calendar.updatedAt = new Date().toISOString(); appendAudit(draft, { actorId: currentUser?.id, companyId: calendar.companyId, action: 'HOLIDAY_REMOVED', entityType: 'HOLIDAY_CALENDAR', entityId: calendar.id, oldValue: before, newValue: calendar }); return calendar; });
    return ok(updated);
  }
  const holidayCopyMatch = path.match(/^\/holiday-calendars\/([^/]+)\/copy$/);
  if (holidayCopyMatch && method === 'POST') {
    const body = await bodyOf(request); const source = (state.holidayCalendars as Row[]).find((item) => item.id === holidayCopyMatch[1]); const targetYear = Number(body.targetYear);
    if (!source) return fail(404, 'NOT_FOUND', 'Holiday calendar not found.');
    if (!targetYear) return fail(422, 'VALIDATION_ERROR', 'Target year is required.');
    if ((state.holidayCalendars as Row[]).some((item) => item.companyId === source.companyId && Number(item.year) === targetYear && item.stateCode === source.stateCode)) return fail(409, 'HOLIDAY_CALENDAR_EXISTS', `A matching calendar already exists for ${targetYear}.`);
    const created = await updateMockState((draft) => { const now = new Date().toISOString(); const calendar = { ...structuredClone(source), id: id('holiday_calendar'), name: `${source.name} ${targetYear}`, year: targetYear, version: 1, createdAt: now, updatedAt: now, holidays: source.holidays.map((holiday: Row) => ({ ...holiday, id: id('holiday'), date: `${targetYear}-${String(holiday.date).slice(5)}` })) }; draft.holidayCalendars.unshift(calendar); appendAudit(draft, { actorId: currentUser?.id, companyId: calendar.companyId, action: 'HOLIDAY_CALENDAR_COPIED', entityType: 'HOLIDAY_CALENDAR', entityId: calendar.id, newValue: calendar }); return calendar; });
    return ok(created, 201);
  }

  if ((path === '/attendance-regularizations' || path === '/attendance-regularizations/mine') && method === 'GET') { const employee = (state.employees as Row[]).find((item) => item.userId === currentUser?.id); const rows = (state.attendanceRegularizations as Row[]).filter((item) => path.endsWith('/mine') ? item.employeeId === employee?.id : recordWithinScope(state, currentUser, item, 'ATTENDANCE.VIEW')).map((item) => { const owner = (state.employees as Row[]).find((candidate) => candidate.id === item.employeeId); return { ...item, employee: owner ? { id: owner.id, employeeId: owner.employeeId, department: owner.department, user: { name: (owner.user as Row)?.name ?? 'Employee' } } : undefined }; }); return ok({ regularizations: rows }); }
  ...
  if (path === '/leave-requests' && method === 'GET') { const search = String(url.searchParams.get('search') || '').toLowerCase(); const status = String(url.searchParams.get('status') || '').toUpperCase(); const leaveType = String(url.searchParams.get('leaveType') || '').toLowerCase(); const allRows = (state.leaves as Row[]).filter((item) => recordWithinScope(state, currentUser, item, 'LEAVE_MANAGEMENT.VIEW')).map((item) => leaveView(state, item)); const rows = allRows.filter((item) => { const name = String((item.employee as Row)?.user?.name ?? '').toLowerCase(); const employeeId = String((item.employee as Row)?.employeeId ?? '').toLowerCase(); return (!search || name.includes(search) || employeeId.includes(search)) && (!status || status === 'ALL' || String(item.status).toUpperCase() === status) && (!leaveType || leaveType === 'all' || String(item.leaveType).toLowerCase() === leaveType || `${String(item.leaveType).toLowerCase()} leave` === leaveType); }); const result = page(rows, url); return ok({ leaves: result.rows, pagination: result.pagination, stats: { total: allRows.length, pending: allRows.filter((item) => item.status === 'PENDING').length, approved: allRows.filter((item) => item.status === 'APPROVED').length, rejected: allRows.filter((item) => item.status === 'REJECTED').length, filtered: rows.length !== allRows.length } }); }
  ...
  if (path === '/salaries' && method === 'GET') { const search = String(url.searchParams.get('search') || '').toLowerCase(); const employmentType = url.searchParams.get('employmentType'); const department = url.searchParams.get('department'); const rows = (state.salaries as Row[]).map((salary) => { const employee = state.employees.find((item) => item.id === salary.employeeId) as Row; return { ...salary, employeeId: employee.employeeId, name: (employee.user as Row).name, department: employee.department, designation: employee.designation, employmentType: employee.employmentType }; }).filter((salary) => (!search || String(salary.name).toLowerCase().includes(search) || String(salary.employeeId).toLowerCase().includes(search)) && (!employmentType || salary.employmentType === employmentType) && (!department || salary.department === department)); const result = page(rows, url); return ok({ results: result.rows, pagination: result.pagination }); }
  if (path === '/payroll-compliance' && method === 'GET') { const company = companyFor(state, currentUser, url); const config = (state.payrollCompliance as Row[]).find((item) => item.companyId === company?.id); if (config) return ok(config); const now = new Date(); const financialYearStart = now.getMonth() < 3 ? now.getFullYear() - 1 : now.getFullYear(); return ok({ id: `payroll_compliance_${company?.id ?? 'new'}`, companyId: company?.id, version: 0, effectiveFrom: `${financialYearStart}-04-01`, stateCode: 'MH', pf: { enabled: true, employeeGroup: 'All Employees', employeeRate: 12, employerRate: 12, wageCeiling: 15000 }, esi: { enabled: true, employeeGroup: 'All Employees', employeeRate: 0.75, employerRate: 3.25, wageCeiling: 21000 }, professionalTax: { enabled: true, employeeGroup: 'All Employees', stateCode: 'MH' }, labourWelfareFund: { enabled: false, employeeGroup: 'All Employees', stateCode: 'MH' }, tds: { enabled: true, employeeGroup: 'All Employees', defaultRegime: 'NEW' }, updatedAt: now.toISOString() }); }
  if (path === '/payroll-compliance' && method === 'PUT') { const body = await bodyOf(request); const company = companyFor(state, currentUser, url); const found = (state.payrollCompliance as Row[]).find((item) => item.companyId === company?.id); if (found && Number(body.version) !== Number(found.version)) return fail(409, 'VERSION_CONFLICT', 'Compliance settings changed in another session. Refresh and try again.'); const saved = await updateMockState((draft) => { let config = (draft.payrollCompliance as Row[]).find((item) => item.companyId === company?.id); const before = config ? structuredClone(config) : null; if (!config) { config = { id: id('payroll_compliance'), companyId: company?.id, version: 0 }; draft.payrollCompliance.push(config); } Object.assign(config, body, { companyId: company?.id, version: Number(config.version) + 1, updatedAt: new Date().toISOString() }); appendAudit(draft, { actorId: currentUser?.id, companyId: company?.id, action: 'PAYROLL_COMPLIANCE_UPDATED', entityType: 'PAYROLL_COMPLIANCE', entityId: config.id, oldValue: before, newValue: config }); return config; }); return ok(saved); }
  ...
  if ((path === '/payslips' || path === '/payslips/mine') && method === 'GET') { const employee = state.employees.find((item) => item.userId === currentUser?.id) as Row | undefined; const selfOnly = path === '/payslips/mine' || currentUser?.role === 'EMPLOYEE'; const month = Number(url.searchParams.get('month') || 0); const year = Number(url.searchParams.get('year') || 0); const search = String(url.searchParams.get('search') || '').toLowerCase(); const department = url.searchParams.get('department'); const employmentType = url.searchParams.get('employmentType'); const rows = (state.payslips as Row[]).filter((item) => { const owner = (state.employees as Row[]).find((candidate) => candidate.id === item.employeeId); const ownerName = String((owner?.user as Row)?.name ?? '').toLowerCase(); return (!selfOnly || (item.employeeId === employee?.id && ['PUBLISHED', 'PAID'].includes(String(item.status).toUpperCase()))) && (!month || Number(item.month) === month) && (!year || Number(item.year) === year) && (!search || ownerName.includes(search) || String(owner?.employeeId ?? '').toLowerCase().includes(search)) && (!department || owner?.department === department) && (!employmentType || owner?.employmentType === employmentType); }); if (path.endsWith('/mine')) return ok(rows); const result = page(rows.map((item) => payslipView(state, item)), url); return ok({ payslips: result.rows, pagination: result.pagination }); }
  const payslipDownload = path.match(/^\/payslips(?:\/mine)?\/([^/]+)\/download$/); if (payslipDownload && method === 'GET') { const payslip = (state.payslips as Row[]).find((item) => item.id === payslipDownload[1]); if (!payslip) return fail(404, 'NOT_FOUND', 'Payslip not found.'); const employee = state.employees.find((item) => item.userId === currentUser?.id) as Row | undefined; if (currentUser?.role === 'EMPLOYEE' && payslip.employeeId !== employee?.id) return fail(403, 'SCOPE_DENIED', 'You can only download your own payslips.'); if (!['PUBLISHED', 'PAID'].includes(String(payslip.status).toUpperCase())) return fail(409, 'PAYSLIP_NOT_PUBLISHED', 'This payslip has not been published yet.'); return new HttpResponse(new Blob([`VOOK demo payslip ${payslipDownload[1]}\nThis is not a tax document.`], { type: 'application/pdf' }), { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `attachment; filename="${payslipDownload[1]}.pdf"` } }); }
const payslipAction = path.match(/^\/payslips\/([^/]+)\/(pay|recalculate)$/); if (payslipAction && method === 'POST') { const found = (state.payslips as Row[]).find((item) => item.id === payslipAction[1]); if (!found) return fail(404, 'NOT_FOUND', 'Payslip not found.'); if (payslipAction[2] === 'recalculate' && ['PUBLISHED', 'PAID'].includes(found.status)) return fail(409, 'PAYSLIP_IMMUTABLE', 'Published historical payslips cannot be recalculated.'); const updated = await updateMockState((draft) => { const payslip = (draft.payslips as Row[]).find((item) => item.id === payslipAction[1]); const before = structuredClone(payslip); if (payslipAction[2] === 'pay') { payslip.status = 'PAID'; payslip.paymentStatus = 'PAID'; payslip.paidAt = new Date().toISOString(); } appendAudit(draft, { actorId: currentUser?.id, companyId: payslip.companyId, action: payslipAction[2] === 'pay' ? 'PAYSLIP_PAID' : 'PAYSLIP_RECALCULATED', entityType: 'PAYSLIP', entityId: payslip.id, oldValue: before, newValue: payslip }); return payslipView(draft, payslip); }); return ok(updated); }
  if (path === '/payroll-runs' && method === 'GET') {
    const company = companyFor(state, currentUser, url);
    const permissions = effectivePermissions(state, currentUser);
    const allowed = (permission: string) => permissions.includes('*') || permissions.includes(permission);
    const rows = (state.payrollRuns as Row[]).filter((item) => item.companyId === company?.id).map((item) => {
      const payslips = (state.payslips as Row[]).filter((payslip) => (item.payslipIds as string[] | undefined)?.includes(String(payslip.id)));
      const permittedActions = [
        ['review', ['READY', 'CALCULATED'].includes(String(item.status)) && allowed('PAYROLL.PROCESS')],
        ['approve', item.status === 'UNDER_REVIEW' && allowed('PAYROLL.APPROVE')],
        ['finalize', item.status === 'APPROVED' && allowed('PAYROLL.FINALIZE')],
        ['publish', item.status === 'FINALIZED' && allowed('PAYROLL.PUBLISH')],
      ].filter((entry) => entry[1]).map((entry) => entry[0]);
      return {
        ...item,
        employeeCount: payslips.length,
        grossPayMinor: Math.round(payslips.reduce((total, payslip) => total + Number(payslip.grossSalary ?? 0), 0) * 100),
        deductionsMinor: Math.round(payslips.reduce((total, payslip) => total + Number(payslip.totalDeductions ?? 0), 0) * 100),
        totalNetMinor: Math.round(payslips.reduce((total, payslip) => total + Number(payslip.netPay ?? 0), 0) * 100),
        version: Number(item.version ?? 1),
        updatedAt: item.updatedAt ?? item.createdAt,
        permittedActions,
      };
    });
    return ok(rows.sort((a, b) => Number(b.year) - Number(a.year) || Number(b.month) - Number(a.month)));
  }
  if (path === '/payroll-runs' && method === 'POST') {
    const body = await bodyOf(request);
    const permissions = effectivePermissions(state, currentUser);
    if (!permissions.includes('*') && !permissions.includes('PAYROLL.PROCESS')) {
      return fail(403, 'PERMISSION_DENIED', 'Your role does not grant payroll processing.');
    }
    const company = companyFor(state, currentUser, url);
    const attendancePeriod = (state.attendancePeriods as Row[]).find((item) => item.companyId === company?.id && item.month === Number(body.month) && item.year === Number(body.year));
    if (attendancePeriod?.status !== 'LOCKED') return fail(422, 'ATTENDANCE_PERIOD_NOT_LOCKED', 'Lock the selected attendance period before preparing payroll.', { month: body.month, year: body.year });
    const unresolvedAttendance = (state.attendanceRegularizations as Row[]).filter((item) => item.companyId === company?.id && String(item.date).startsWith(`${body.year}-${String(body.month).padStart(2, '0')}`) && !['APPROVED', 'REJECTED'].includes(String(item.status).toUpperCase()));
    if (unresolvedAttendance.length) return fail(422, 'ATTENDANCE_EXCEPTIONS_OPEN', 'Resolve or reject all attendance regularizations before preparing payroll.', { count: unresolvedAttendance.length });
    if ((state.payrollRuns as Row[]).some((item) => item.companyId === company?.id && item.month === Number(body.month) && item.year === Number(body.year) && item.status !== 'CANCELLED')) return fail(409, 'PAYROLL_PERIOD_EXISTS', 'A payroll run already exists for this period.');
    const requestedIds = Array.isArray(body.employeeIds) && body.employeeIds.length ? new Set(body.employeeIds) : null;
    const selectedEmployees = (state.employees as Row[]).filter((item) => item.companyId === company?.id && item.status !== 'EXITED' && (!requestedIds || requestedIds.has(item.id)));
    const issues = selectedEmployees.flatMap((employee) => {
      const salary = (state.salaries as Row[]).find((item) => item.employeeId === employee.id);
      const employeeIssues = [];
      if (!salary || Number(salary.annualCtc ?? employee.annualCtc) <= 0) employeeIssues.push({ employeeId: employee.id, name: (employee.user as Row)?.name, source: 'SALARY', reason: 'No effective salary structure is configured.' });
      if (!employee.joiningDate) employeeIssues.push({ employeeId: employee.id, name: (employee.user as Row)?.name, source: 'EMPLOYEE', reason: 'Joining date is missing.' });
      return employeeIssues;
    });
    if (issues.length) return fail(422, 'PAYROLL_PREFLIGHT_FAILED', 'Resolve payroll blockers before creating the run.', issues);
    const created = await updateMockState((draft) => {
      const month = Number(body.month); const year = Number(body.year); const monthDays = new Date(year, month, 0).getDate();
      const compliance = (draft.payrollCompliance as Row[]).find((item) => item.companyId === company?.id) ?? {};
      const payslips = selectedEmployees.map((employee) => {
        const salary = (draft.salaries as Row[]).find((item) => item.employeeId === employee.id) as Row;
        const annualCtc = Number(salary.annualCtc ?? employee.annualCtc);
        const basicSalary = Number(salary.basicAnnual ?? annualCtc * .5) / 12;
        const allowances = Number(salary.allowancesAnnual ?? annualCtc * .15) / 12;
        const configuredDeductions = Number(salary.deductionsAnnual ?? 0) / 12;
        const records = (draft.attendance as Row[]).filter((item) => item.employeeId === employee.id && String(item.date).startsWith(`${year}-${String(month).padStart(2, '0')}`));
        const absentDays = records.filter((item) => ['ABSENT', 'LEAVE'].includes(String(item.status).toUpperCase())).length;
        const joiningDate = new Date(`${employee.joiningDate}T00:00:00`); const periodStart = new Date(year, month - 1, 1); const periodEnd = new Date(year, month, 0);
        const eligibleStart = joiningDate > periodStart ? joiningDate : periodStart; const eligibleDays = joiningDate > periodEnd ? 0 : monthDays - eligibleStart.getDate() + 1;
        const payableDays = Math.max(0, eligibleDays - absentDays);
        const prorate = payableDays / monthDays; const grossSalary = (basicSalary + allowances) * prorate;
        const pfBase = Math.min(basicSalary * prorate, Number(compliance.pf?.wageCeiling ?? basicSalary)); const pf = compliance.pf?.enabled ? pfBase * Number(compliance.pf.employeeRate ?? 0) / 100 : 0;
        const esi = compliance.esi?.enabled && grossSalary <= Number(compliance.esi.wageCeiling ?? 0) ? grossSalary * Number(compliance.esi.employeeRate ?? 0) / 100 : 0;
        const totalDeductions = configuredDeductions * prorate + pf + esi; const netPay = Math.max(0, grossSalary - totalDeductions);
        const existing = (draft.payslips as Row[]).find((item) => item.companyId === company?.id && item.employeeId === employee.id && item.month === month && item.year === year);
        const payslip = existing ?? { id: id('payslip'), companyId: company?.id, employeeId: employee.id, payslipId: `PS-${year}${String(month).padStart(2, '0')}-${employee.employeeId}`, month, year, period: new Intl.DateTimeFormat('en', { month: 'long', year: 'numeric' }).format(periodStart), createdAt: new Date().toISOString() };
        Object.assign(payslip, { status: 'DRAFT', paymentStatus: 'NOT_RECORDED', grossSalary: Math.round(grossSalary), totalDeductions: Math.round(totalDeductions), netPay: Math.round(netPay), snapshot: { calculationVersion: 'v2', annualCtc, monthDays, payableDays, absentDays, basicSalary: Math.round(basicSalary * prorate), allowances: Math.round(allowances * prorate), configuredDeductions: Math.round(configuredDeductions * prorate), pf: Math.round(pf), esi: Math.round(esi), overtimeMinutes: 0, overtimePay: 0, reimbursementAmount: 0 } });
        if (!existing) draft.payslips.unshift(payslip);
        return payslip;
      });
      const run = { id: id('payroll_run'), companyId: company?.id, month, year, status: 'READY', version: 1, preparedBy: currentUser?.id, approvedBy: null, finalizedAt: null, publishedAt: null, payslipIds: payslips.map((item) => item.id), employeeCount: payslips.length, grossPayMinor: payslips.reduce((sum, item) => sum + Math.round(Number(item.grossSalary ?? 0) * 100), 0), deductionsMinor: payslips.reduce((sum, item) => sum + Math.round(Number(item.totalDeductions ?? 0) * 100), 0), totalNetMinor: payslips.reduce((sum, item) => sum + Math.round(Number(item.netPay ?? 0) * 100), 0), currency: 'INR', validationIssues: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
      draft.payrollRuns.unshift(run);
      appendAudit(draft, { actorId: currentUser?.id, companyId: company?.id, action: 'PAYROLL_CALCULATED', entityType: 'PAYROLL_RUN', entityId: run.id, newValue: run });
      return { message: 'Payroll calculated and ready for review.', period: `${body.month}/${body.year}`, month: body.month, year: body.year, created: payslips.length, skipped: selectedEmployees.length - payslips.length, runId: run.id, status: run.status };
    });
    return ok(created, 201);
  }
  const payrollAction = path.match(/^\/payroll-runs\/([^/]+)\/(review|approve|finalize|publish)$/);
  if (payrollAction && method === 'POST') {
    const body = await bodyOf(request);
    const run = (state.payrollRuns as Row[]).find((item) => item.id === payrollAction[1]);
    if (!run) return fail(404, 'NOT_FOUND', 'Payroll run not found.');
    if (run.companyId !== currentUser?.companyId && currentUser?.role !== 'SUPER_ADMIN') return fail(403, 'SCOPE_DENIED', 'This payroll run belongs to another company.');
    const transitions: Record<string, { from: string[]; to: string; permission: string }> = {
    ...
    const transition = transitions[payrollAction[2]];
    if (Number(body.version) !== Number(run.version ?? 1)) return fail(409, 'VERSION_CONFLICT', 'This payroll run changed after you opened it. Refresh and review the latest version.');
    if (!String(body.reason ?? '').trim()) return fail(422, 'AUDIT_REASON_REQUIRED', 'Add a reason before changing the payroll status.');
    if (!transition.from.includes(run.status)) return fail(409, 'INVALID_PAYROLL_TRANSITION', `Payroll cannot move from ${run.status} to ${transition.to}.`);
    const permissions = effectivePermissions(state, currentUser);
    if (!permissions.includes('*') && !permissions.includes(transition.permission)) return fail(403, 'PERMISSION_DENIED', `Your role does not grant ${transition.permission}.`);
    const updated = await updateMockState((draft) => {
      const payroll = (draft.payrollRuns as Row[]).find((item) => item.id === run.id);
      const before = structuredClone(payroll);
      payroll.status = transition.to;
      payroll.version = Number(payroll.version ?? 1) + 1;
      payroll.updatedAt = new Date().toISOString();
      if (payrollAction[2] === 'approve') payroll.approvedBy = currentUser?.id;
      if (payrollAction[2] === 'finalize') payroll.finalizedAt = new Date().toISOString();
      if (payrollAction[2] === 'publish') {
        payroll.publishedAt = new Date().toISOString();
        (draft.payslips as Row[]).filter((item) => payroll.payslipIds.includes(item.id)).forEach((item) => {
          item.status = 'PUBLISHED';
          const employee = (draft.employees as Row[]).find((employeeRow) => employeeRow.id === item.employeeId);
          if (employee?.userId) draft.notifications.unshift({ id: id('notification'), userId: employee.userId, type: 'PAYROLL', title: 'Payslip published', message: `Your payslip for ${payroll.month}/${payroll.year} is available.`, isRead: false, data: { payslipId: item.id }, createdAt: new Date().toISOString() });
        });
      }
      appendAudit(draft, { actorId: currentUser?.id, companyId: payroll.companyId, action: `PAYROLL_${transition.to}`, entityType: 'PAYROLL_RUN', entityId: payroll.id, oldValue: before, newValue: payroll, reason: String(body.reason).trim() });
      return payroll;
    });
    return ok(updated);
  }

  if ((path === '/expenses' || path === '/expenses/mine') && method === 'GET') {
    const employee = state.employees.find((item) => item.userId === currentUser?.id) as Row | undefined;
    const scoped = (state.expenses as Row[]).filter((item) => path === '/expenses' || item.employeeId === employee?.id);
    const search = String(url.searchParams.get('search') ?? '').toLowerCase();
    const status = String(url.searchParams.get('status') ?? '').toUpperCase();
    const rows = scoped.filter((item) => {
      const owner = (state.employees as Row[]).find((candidate) => candidate.id === item.employeeId);
      const ownerName = String((owner?.user as Row)?.name ?? '').toLowerCase();
      const normalized = String(item.status).toUpperCase();
      const statusMatch = !status || status === 'ALL' || (status === 'PENDING' && ['PENDING', 'SUBMITTED', 'MANAGER_APPROVED'].includes(normalized)) || (status === 'APPROVED' && ['APPROVED', 'FINANCE_APPROVED'].includes(normalized)) || (status === 'COMPLETED' && ['PAID', 'REJECTED'].includes(normalized)) || normalized === status;
      return (!search || ownerName.includes(search) || String(owner?.employeeId ?? '').toLowerCase().includes(search) || String(item.category).toLowerCase().includes(search)) && statusMatch;
    });
    const stats = { pending: scoped.filter((item) => ['PENDING', 'SUBMITTED', 'MANAGER_APPROVED'].includes(String(item.status).toUpperCase())).length, approved: scoped.filter((item) => ['APPROVED', 'FINANCE_APPROVED', 'PAID'].includes(String(item.status).toUpperCase())).length, rejected: scoped.filter((item) => String(item.status).toUpperCase() === 'REJECTED').length, total: scoped.reduce((sum, item) => sum + Number(item.amount), 0) };
    if (path.endsWith('/mine')) return ok({ expenses: rows, stats });
    const result = page(rows.map((item) => expenseView(state, item)), url);
    return ok({ expenses: result.rows, pagination: result.pagination, stats });
  }
