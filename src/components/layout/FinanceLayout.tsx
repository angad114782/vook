...
function TopBar({ onOpenNavigation }: { onOpenNavigation: () => void }) {
  ...
  return (
    <div className="app-topbar" style={{ height: '60px', minHeight: '60px', backgroundColor: 'white', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '16px' }}>
      ...
      <div className="app-topbar__profile" onClick={() => navigate('/finance/settings')} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
        <div className="app-topbar__profile-copy" style={{ textAlign: 'right' }}>
            <p style={{ fontSize: '13px', fontWeight: 700, color: '#0f172a' }}>{user?.name ?? 'Finance'}</p>
            <p style={{ fontSize: '11px', color: '#64748b' }}>Human Resource</p>
          </div>
...
export default function FinanceLayout() {
  ...
  if (user.role !== 'FINANCE' && user.role !== 'SUPER_ADMIN') return <Navigate to="/login" replace />;

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'Inter, sans-serif', backgroundColor: '#f8fafc' }}>
      <FinanceSidebar mobileOpen={mobileNavOpen} onMobileClose={() => setMobileNavOpen(false)} />
