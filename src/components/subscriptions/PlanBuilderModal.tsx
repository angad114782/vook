...
const ids = (items: PlanData["moduleIds"] | undefined) =>
...
export default function PlanBuilderModal({ plan, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    subscriptionsApi
      ...
      .then(({ data }) => {
        ...
        setForm((current) => ({
          ...current,
          moduleIds: [...new Set([...core, ...current.moduleIds])],
        }));
      })
      .catch(() => setError("The module catalogue could not be loaded."))
  ...
  const groups = useMemo(() => {
    ...
    return Object.entries(
      visible.reduce<Record<string, ModuleCatalogItem[]>>((result, item) => {
  ...
  const toggleModule = (module: ModuleCatalogItem) => {
    ...
    setForm((current) => ({
      ...current,
      moduleIds: current.moduleIds.includes(module.id)
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(extractError(caught, "The plan draft could not be saved."));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
        <form onSubmit={save}>
          ...
          <section>
            <div className="plan-builder__section-title">
              <strong>1. Identity and billing</strong>
              <span>What customers see and pay</span>
            </div>
            <div className="plan-builder__grid plan-builder__grid--4">
              ...
              <label className="admin-label">
                ...
                <input
                  ...
                  value={form.type}
                  onChange={(event) =>
                    setForm({ ...form, type: event.target.value.toUpperCase() })
                  }
                  placeholder="BUSINESS"
                />
              </label>
            ...
            <div className="plan-builder__grid plan-builder__grid--2">
              ...
              <label className="admin-label">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, storageGB: Number(event.target.value) })
                  }
                />
              </label>
            </div>
          ...
          <section>
            <div className="plan-builder__section-title">
              <strong>3. Included modules</strong>
              <span>
                {form.moduleIds.length} selected · core controls stay included
              </span>
            </div>
            <div className="plan-builder__search">
              <Search size={15} />
              <input
                value={search}
                      ...
                      {modules.map((module) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={module.isCore || !module.planSelectable}
                          >
                            <span className="plan-module-check">
                              {module.isCore ? (
                                <LockKeyhole size={13} />
                              ) : selected ? (
                                <Check size={14} />
          ...
          <section>
            <div className="plan-builder__section-title">
              <strong>4. Plan highlights</strong>
              <span>Plain-language details displayed during selection</span>
            </div>
            <div className="plan-builder__feature-entry">
              <input
                className="admin-input"
                value={feature}
                ...
                placeholder="Example: Multi-level approval workflows"
              />
              <button
                type="button"
            ...
            <div className="plan-builder__features">
          ...
          <footer>
            <div>
              <strong>
                {plan
                  ? "Saving creates an unpublished revision."
                  : "New plans start as drafts."}
              </strong>
              <span>Companies only receive immutable published versions.</span>
            </div>
            <button
              type="button"
              className="admin-button admin-button--secondary"
            ...
            >
              {saving && <Loader2 size={15} className="spin" />} Save draft
            </button>
          </footer>
        </form>
    </AppDrawer>
