...
import { ArrowLeft, ArrowRight, Building2, Check, Circle, MapPin, SkipForward } from 'lucide-react';
...
const STEP_META = [
  ...
  ['first-branch', 'First branch', 'Location, timezone, and geofence'],
  ['organization', 'Organization', 'Departments, designations, and reporting'],
  ['roles', 'Roles and access', 'Scoped assignments by company, branch, department, or team'],
  ['shifts-holidays', 'Shifts and holidays', 'Working templates and calendars'],
  ['attendance-policy', 'Attendance policy', 'Verification and safety rules'],
...
export default function CAOnboardingPage() {
  ...
  const [branch, setBranch] = useState({ name: 'Head Office', code: 'HO', address: '', timezone: 'Asia/Kolkata', geofenceRadiusMeters: 200 });
  ...
  const completeCount = steps.filter((step) => step.status === 'COMPLETED' || step.status === 'SKIPPED').length + [...lockedSteps].filter((key) => !steps.some((step) => step.key === key && ['COMPLETED', 'SKIPPED'].includes(step.status))).length;
  ...
  const requiredReady = useMemo(() => ['company-profile', 'first-branch'].every((key) => steps.find((step) => step.key === key)?.status === 'COMPLETED'), [steps]);
  const save = () => mutation.mutate({ key: active, status: 'COMPLETED', data: active === 'company-profile' ? profile : active === 'first-branch' ? branch : undefined });
  ...
  return <div className="admin-page onboarding-page">
    ...
    <div className="onboarding-layout">
      ...
      <section className="admin-card onboarding-form">
        ...
        {active === 'first-branch' && <><div className="form-title"><MapPin size={19} /><div><h2>First branch</h2><p>A branch is required for attendance and workforce scope.</p></div></div><div className="form-grid"><label className="admin-label">Branch name<input className="admin-input" value={branch.name} onChange={(event) => setBranch({ ...branch, name: event.target.value })} /></label><label className="admin-label">Code<input className="admin-input" value={branch.code} onChange={(event) => setBranch({ ...branch, code: event.target.value.toUpperCase() })} /></label><label className="admin-label">Address<input className="admin-input" value={branch.address} onChange={(event) => setBranch({ ...branch, address: event.target.value })} /></label><label className="admin-label">Timezone<select className="admin-input" value={branch.timezone} onChange={(event) => setBranch({ ...branch, timezone: event.target.value })}><option>Asia/Kolkata</option><option>Asia/Dubai</option><option>Europe/London</option></select></label><label className="admin-label">Geofence radius (metres)<input className="admin-input" type="number" min={25} max={50000} value={branch.geofenceRadiusMeters} onChange={(event) => setBranch({ ...branch, geofenceRadiusMeters: Number(event.target.value) })} /></label></div></>}
        {!['company-profile', 'first-branch'].includes(active) && <div className="setup-detail"><h2>{STEP_META[selectedIndex]?.[1]}</h2><p>{selectedLocked ? `${stepModules[active]} is not included in the company's pinned plan version. Existing permissions stay dormant and this step is excluded from completion.` : `${STEP_META[selectedIndex]?.[2]}. Save this checkpoint now, or skip it and return from the dashboard checklist.`}</p>{deepLinks[active] && !selectedLocked && <button className="admin-button admin-button--secondary" disabled={!requiredReady} onClick={() => navigate(deepLinks[active])}>Open {STEP_META[selectedIndex]?.[1]} <ArrowRight size={16} /></button>}{!requiredReady && deepLinks[active] && !selectedLocked && <small>Complete the company profile and first branch before opening operational settings.</small>}{selectedLocked && <button className="admin-button admin-button--secondary" onClick={() => navigate('/company-admin/modules')}>Review plan modules</button>}</div>}
        <footer className="setup-actions"><button className="admin-button admin-button--secondary" disabled={selectedIndex === 0} onClick={() => setActive(STEP_META[Math.max(0, selectedIndex - 1)][0])}><ArrowLeft size={16} /> Back</button><div>{!selectedLocked && !['company-profile', 'first-branch'].includes(active) && <button className="admin-button admin-button--secondary" onClick={() => mutation.mutate({ key: active, status: 'SKIPPED' })}><SkipForward size={16} /> Skip for now</button>}<button className="admin-button" onClick={selectedLocked ? () => setActive(STEP_META[Math.min(STEP_META.length - 1, selectedIndex + 1)][0]) : save}>{selectedLocked ? 'Continue' : active === 'invitations' ? 'Finish setup' : 'Save and continue'} <ArrowRight size={16} /></button></div></footer>
      </section>
    </div>
  </div>;
}
