  ...
  return (
    <div className="admin-page">
      <header className="admin-page__header">
        <div><h1>Announcements</h1><p>Send targeted platform announcements and review delivery and read coverage.</p></div>
      ...
      <div className="admin-split-layout">
        <form className="admin-card admin-form-card" onSubmit={send}>
          ...
          <div className="admin-form-actions"><button className="admin-button" disabled={sending}><Send size={15} /> {sending ? 'Sending…' : 'Send announcement'}</button></div>
        </form>

        <section className="admin-card admin-list-card">
          <h2>Recent announcements</h2>
          {broadcasts.isLoading && <div className="empty-state">Loading broadcasts…</div>}
          {!broadcasts.isLoading && !broadcasts.data?.length && <div className="empty-state"><BellRing size={24} /><strong>No announcements yet</strong></div>}
