...
export default function PaymentsPage() {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
    ...
    <div className="responsive-stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>{statCards.map(({ label, value, icon: Icon, color, bg }) => <div key={label} className="responsive-stat-card" style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '14px 16px', background: 'white', border: '1px solid #e2e8f0', borderRadius: 10 }}><span style={{ width: 32, height: 32, display: 'grid', placeItems: 'center', borderRadius: 8, color, background: bg }}><Icon size={16} /></span><div><p style={{ margin: 0, fontSize: 19, fontWeight: 750, color: '#0f172a' }}>{value}</p><p style={{ margin: '2px 0 0', color: '#64748b', fontSize: 11 }}>{label}</p></div></div>)}</div>
    ...
    <div style={{ overflowX: 'auto', background: 'white', border: '1px solid #e2e8f0', borderRadius: 10 }}><ResponsiveTable style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}><thead><tr>{['Company', 'Source', 'Plan', 'Amount', 'Status', 'Reference', 'Date'].map((heading) => <th key={heading} style={{ padding: '12px 14px', textAlign: 'left', color: '#64748b', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', fontSize: 11, fontWeight: 750, textTransform: 'uppercase', letterSpacing: '.3px' }}>{heading}</th>)}</tr></thead><tbody>
