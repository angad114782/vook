...
function GeneralTab({ showToast }: { showToast: (m: string, t: 'success' | 'error') => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await platformApi.updateSettings('general', form); showToast('General settings saved successfully', 'success'); }
...
function SecurityTab({ showToast }: { showToast: (m: string, t: 'success' | 'error') => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await platformApi.updateSettings('security', { twoFA, sessionTimeout }); showToast('Security settings saved successfully', 'success'); }
...
function NotificationsTab({ showToast }: { showToast: (m: string, t: 'success' | 'error') => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await platformApi.updateSettings('notifications', prefs); showToast('Notification preferences saved', 'success'); }
...
function SystemConfigTab({ showToast }: { showToast: (m: string, t: 'success' | 'error') => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await platformApi.updateSettings('system', { maxUsers: Number(maxUsers), maintenance, maintenanceMsg }); showToast('System configuration updated', 'success'); }
